# 부적절 처방 선별 및 Deprescribing — 캐나다 일차의료 가이드라인 [CLINICAL]

tags: [CLINICAL, REGULATORY]
keywords: deprescribing, 부적절 처방, PIP, STOPP, START, 노인 다약제, polypharmacy, medication review, 처방 최적화

version: (미정)
supersedes: (미정)
freshness.primarySourceYear: 2026
applicability: ≥65세 외래 환자 — 다약제 처방 재검토
parents: []
relations: [[[diabetes-deprescribing-lifestyle]], [[elderly-psychotropic-deprescribing]], [[frailty]], [[palliative-pain]]]

> primarySources (Tier 1):
> - McDonald EG et al. Screening for potentially inappropriate prescribing in primary care: Canadian guideline. Can Fam Physician. 2026 Mar 17;72(3):173-178. PMID:41844291, DOI:10.46747/cfp.7203173

---

## 핵심 권고 (protocol)

> [출처: McDonald EG et al. Can Fam Physician 2026 Mar;72(3):173-178. PMID:41844291]
> 2개 체계적 고찰 기반; 캐나다 C-64 법안(적정 약물사용 전략)과 연계

**권고 1 (강력권고, 중등도 근거):**
≥65세 성인은 약물 적절성 최적화를 위한 **처방 점검(Prescription Checkup) 또는 관련 중재**를 받아야 한다.

- 효과적 중재 형태: 처방의·약사가 **구조적 접근법 또는 규칙 기반 도구**를 사용하는 약물 재검토
- 도구 예: STOPP/START, Beers Criteria, RAMQ 적절성 도구

**권고 2 (강력권고, 중등도 근거):**
정부는 약물 적절성 최적화 중재를 **재정 지원**해야 한다.

---

## 처방 점검 실전 (protocol)

### 구조적 접근 도구

| 도구 | 대상 | 활용 |
|---|---|---|
| **STOPP/START** | ≥65세 | 부적절 처방(STOPP) + 누락 처방(START) 동시 검출 |
| **Beers Criteria** | ≥65세 (미국) | 노인 부적절 약물 목록 |
| **STOPP-Frail** | 허약 노인 | 완화 목적 처방 최소화 |

### 일차의료 외래 적용 프로세스

1. ≥65세 환자 → 연 1회 처방 전체 재검토 루틴화
2. STOPP 도구로 부적절 약물 flagging
3. 처방의·약사 협력: 확인·중단·대체 결정
4. 환자·보호자 상담 (중단 이유, 기대 효과 설명)
5. 중단 후 추적: 증상 재발·금단 반응 모니터링

---

## 우선 재검토 약물군

| 약물군 | 재검토 이유 |
|---|---|
| PPI | 장기 무증상 예방 사용 → 중단 시도 (GI 증상 재확인) |
| 수면진정제 (BZD·Z-drug) | 낙상·인지저하·의존성 위험 |
| 항콜린 약물 | ACB 1도 낙은 부하도 구강건조·인지 영향 |
| 스타틴 (임종기) | 기대 여명 단기 환자 이익 없음 |
| 경구 혁당강하제 (임종기) | 저혈당 위험 vs 이익 재검토 |
| 항고혈압제 (임종기·낙상 위험) | 기립성 저혈압·낙상 위험 vs 이익 |

---

## 주의사항 (precaution)

- 처방 중단 시 **점진적 감량** — 급격 중단은 금단·반동 위험 (BZD·오피오이드·스테로이드·베타차단제)
- 중단 후 증상 재발 시 재처방 가능 — 중단이 '실패'가 아님
- 환자·가족 설명: "덜 먹는 게 더 건강할 수 있습니다" 메시지

---

## 의뢰 기준 (referral)

- 복잡 다약제(≥10종) + 다중이환 → 노인의학과 협진 (CGA)
- 약물 상호작용 고복잡도 → 임상약사 협진 권고
- 항응고제·항경련제·항정신병약 중단 → 전문과 협의 후 결정

---

## 관련 엔트리

- [[frailty]] — 허약 노인 다약제 deprescribing
- [[palliative-pain]] — 임종기 비필수 약물 중단
- [[diabetes-deprescribing-lifestyle]] — T2DM 감약 (생활습관 의학)
- [[elderly-psychotropic-deprescribing]] — 노인 항우울제·정신과 약물 감약·낙상 예방

> (2026-05-12 Auditor R2 분할; 본 파일에서 T2DM 감약·치매 항우울제·정신과 약물 낙상 → 별도 entry 분리)

---

## 치매 환자 감약 4단계 로드맵 — Johns Hopkins (protocol) [CLINICAL]

> [출처: Green AR et al. Deprescribing for People with Dementia: A Roadmap. Drugs Aging. 2025 Aug 26;42(9):795-806. PMID:40856967, DOI:10.1007/s40266-025-01238-w]
> 리뷰 논문. Johns Hopkins University School of Medicine (Division of Geriatric Medicine and Gerontology). NIH 연구비 지원.

치매 환자(PWD)는 다약제 노출 + 잠재적 부적절 약물이 집중되는 집단. 치매 특유 장벽(행동증상 재발 위험·가변적 경과·보호자 중심 의사결정)을 다루는 감약 로드맵.

**Johns Hopkins 4단계 로드맵:**

| 단계 | 내용 |
|---|---|
| **1단계** | 감약 대상 선별 — 약물 관련 목표 파악 + 위험·이득 trade-off 검토 |
| **2단계** | 감량 계획 수립 — 우선순위 약물 선정, 감량 속도·순서 결정 |
| **3단계** | 사전 조치 완료 — 감약 전 비약물 대안 준비 등 |
| **4단계** | 밀접 추적관찰 — 증상 재발·금단·행동 변화 모니터링 |

**FRAME 보호자 소통 도구 (외래 면담 스크립트로 즉시 사용):**

| 글자 | 의미 | 외래 적용 |
|---|---|---|
| **F**raming | 감약을 "돌봄 개선"으로 프레임 | "약을 줄이면 더 안전하게 지내실 수 있어요" |
| **R**ationale | 감약 근거 설명 | "이 약의 위험이 이익보다 커졌습니다" |
| **A**cknowledgement | 보호자 우려 수용 | "걱정되시는 마음 충분히 이해합니다" |
| **M**aking a plan | 구체적 감량 계획 공동 수립 | 감량 일정·추적 간격 함께 결정 |
| **E**mpathy | 감정적 공감 유지 | 보호자 피로·갈등 인정 |

**치매 환자 우선 감약 대상 약물:**
- 항콜린제 (OAB 약물·항히스타민제·삼환계 항우울제)
- 수면진정제 (BZD·Z-drug)
- 항정신병약 — BPSD 외 적응증 없을 때
- 스타틴·혈압약 (임종기 치매·기대여명 단기)
- 처방 cascade 약물 (ChEI → OAB 항무스카린제 → [[prescribing-cascade]] 참조)

---

## 노인 Z-수면제 간략 개입으로 부적절 사용 감소 (protocol) [CLINICAL — 조건부]

> [출처: Siddiqui TG et al. Brief intervention for inappropriate z-hypnotics use in older adults: a before and after intervention study in primary care. Scand J Prim Health Care. 2026;44(1):2660168. PMID:42031000, DOI:10.1080/02813432.2026.2660168]
> 전후 비교 연구, n=45명 (평균 69.4세, 여성 31명), GP 21명, 노르웨이. ClinicalTrials.gov NCT06032715

**핵심 수치:**

| 지표 | 전(기저) | 후(치료) | 효과 크기 |
|---|---|---|---|
| 부적절 Z-수면제 사용 비율 | **68.9%** | **27.8%** | OR 0.16 (95% CI 0.04–0.65) |
| 수면 점수 (GSAQ-insomnia) | 상승 | **유의 악화 없음** | 유지 |
| 6개월 시점 불면증 | — | **0명** | — |
| 불안·우울 (HADS) | 10.1 | 7.3 | Cohen's d -0.44 |
| 인지기능 (MOCA) | 기저 | 유의 변화 없음 | — |
| 통증 (VAS) | 기저 | 유의 변화 없음 | — |

**부적절 Z-수면제 기준:** ≥4주 사용 + ≥주 3회 (STOPP 기준)

**간략 개입(BI) 구성 — GP 직접 시행:**
1. 현재 수면제 사용 패턴·이유 탐색
2. 장기 Z-수면제 위험 설명 (낙상·인지저하·의존성)
3. 점진적 감량 계획 수립 (환자와 함께)
4. 비약물 수면 전략 안내 (수면위생·인지행동치료)
5. 6주·6개월·12개월 추적 평가

**임상 함의:**
- GP 직접 BI로 수면·인지·통증 악화 없이 Z-수면제 감약 달성 가능
- 환자 설득 근거: "수면이 나빠지지 않는다"는 데이터 직접 제시 가능
- STOPP 기준 수면제 항목 실행 도구로 바로 적용
- 소규모(n=45) 단일국가 연구 — 외삽 시 주의 [CLINICAL — 조건부]

**한국 외래 변환 시 확인:**
- 한국 줄피데안 처방 급여 제한(2주 이상 원칙적 제한) [출처 미확인 — researcher 검증 권장]
- 에스조피클론(루네스타) 한국 급여 기준 [출처 미확인]

---

## 지역사회 환자의 감약 거부 — 예측 인자·면담 전략 (notes) [CLINICAL — 조건부]

> [출처: Marcellaud E et al. Attitudes Toward Deprescribing Among Community-Dwelling Adults. Drugs Aging. 2026 May 2. PMID:42068533, DOI:10.1007/s40266-026-01298-6]
> 단면 조사, n=505명(≥55세, 다약제 ≥5종, 지역약국), 프랑스.

**핵심 수치:**
- 감약 거부률: **24.6%** (1/4 환자)
- 주요 거부 사유: 치료 효과 확신 56.5% / 증상 재발 두려움 35.5%

**거부와 유의하게 연관된 임상 조건:**

| 조건 | OR | 의미 |
|---|---|---|
| 수면장애 | 0.51 (0.33–0.78) | 감약 거부 가능성 2배 ↑ |
| 요실금 | 0.58 (0.34–0.96) | 감약 거부 가능성 1.7배 ↑ |

**감약 수용과 연관된 요인:**

| 요인 | OR | 의미 |
|---|---|---|
| 약물 부담 과다 인지 | 1.82 (1.12–2.95) | 수용 가능성 1.8배 ↑ |
| 일상 제약 부담 | 1.76 (1.07–2.89) | 수용 가능성 1.8배 ↑ |

**외래 감약 면담 전략:**

1. **수면제 감약 제안 시** → 수면 효과 확신 강함 → 선제 공감 + 비약물 수면치료 대안 제시 먼저
2. **요실금 약물 감약 제안 시** → "약 없으면 요실금이 다시 심해질까봐 걱정" 공감 → 대체 전략(골반저운동·미라베그론 등) 먼저 제시
3. **감약 수용도 높은 환자 식별** → 약물 부담 많다고 느끼는 환자 → "드시는 약이 너무 많아 불편하지 않으신가요?" 질문으로 열기
4. "덜 먹는 게 더 건강할 수 있다"는 메시지 + 효과 있는 약은 유지함을 명확히 설명

---

## 항우울제 Deprescribing — 느린 점감 + 심리치료 병행 [CLINICAL — 조건부]

> [출처: Wise J. Antidepressant deprescribing: Slow tapering plus therapy is best way to come off medication, say researchers. BMJ. 2025;391:r2608. PMID:41381099, DOI:10.1136/bmj.r2608]
> BMJ 뉴스 기사 — 연구진 권고 보고; 원 연구논문 전문 미확인 [초록 기반 — 전문 미확인]

### 핵심 권고 — 항우울제 중단 최선 방법

| 방법 | 결과 |
|---|---|
| 급격 중단 (abrupt) | 금단증상(불안·어지럼·전기충격감) + 재발 위험 ↑ |
| 빠른 감량 (fast taper) | 급격 중단보다 낫지만 위험 여전 |
| **느린 점감 (slow hyperbolic tapering)** | **금단증상·재발 위험 최소화** |
| 심리치료 병행 (CBT·지지 상담) | 중단 성공률 ↑ |

### Hyperbolic Tapering 원리

- 세로토닌 수용체 점유는 용량의 **대수(log)적 비례** — 소량 단계에서도 조금씩 감량 필수
- 예: 20mg→10mg 감량보다 2mg→1mg 감량이 수용체 점유 변화 폭이 더 큼
- 임상 적용: **수개월에 걸쳐 소량 단계적 감량** (수주 감량 금지)

### 외래 적용 프로토콜

1. **감량 전 안정 기간 확인** — 우울 증상 재발 없이 ≥6개월 안정 유지
2. **심리치료 병행 시작** — CBT 또는 지지 상담 (감량 개시 전 또는 동시)
3. **점진적 감량 계획** — 수개월 일정; SSRI 저용량 정제 또는 액체형 활용
4. **금단증상 모니터링** — 불안·어지럼·전기충격감·GI 증상; 증상 시 감량 속도 늦추기
5. **"감량 실패"는 없음** — 속도 재조정 옵션; 급재처방보다 속도 조절 우선

### 해당 환자군

- 우울·불안으로 SSRI/SNRI 복용 중 → 6개월+ 안정 후 중단 검토
- STOPP 대상 항우울제 (≥65세 항콜린 부담·낙상 위험 재검토 대상)
- 관련: [[elderly-psychotropic-deprescribing]] — 노인 항우울제 감약 + 낙상 예방

### 한국 외래 변환 시 확인

- SSRI 저용량 분할·액체형 처방 가용성 [출처 미확인 — researcher 검증 권장]
- CBT 급여 현황 [출처 미확인 — researcher 검증 권장]

---

> 정신과 약물(BZD·수면제·항정신병약) 감약 효과·낙상 감소·프로토콜은 [[elderly-psychotropic-deprescribing]] 참조.
> T2DM 감약 실사 근거는 [[diabetes-deprescribing-lifestyle]] 참조.

---

## 가정간호 결정서한의 현실적 한계 — 약물 관리 안전 간극 (notes) [CLINICAL — 조건부]

> [출처: Nilsen MK et al. Scand J Prim Health Care. 2026;44:2669814. PMID:42132490, DOI:10.1080/02813432.2026.2669814]
> 노르웨이 농촌 3개 지자체 질적 사례연구. 15건 결정서한 + 35건 반구조화 인터뷰(환자 9명, GP 1명, 간호사 4명, 서비스 배분 담당자 2명), 12개월.
> [초록 기반 — 전문 미확인]

**핵심 발견:**
- 결정서한(medication management decision letter) → 환자·간호사 모두 **거의 읽지 않음**
- 실무에서는 **케어플랜·업무목록·구두 협의**에 의존
- 모호한 언어 → 해석 편차 → 환자 권리 행사 제한
- 재정 제약이 서비스 배분의 실질적 결정 요인

**재택 다약제 환자 약물 안전 설계 시사점:**

| 현실 장벽 | 대안적 접근 |
|---|---|
| 공식 서류 미독 | 케어플랜에 직접 통합 |
| 간호사 해석 불일치 | 구두 협의 + 케어플랜 일치 확인 |
| 환자 권리 취약 | 환자 참여 재평가 루틴화 |

**Deprescribing 개입 설계 시:**
- 서면 지시 단독보다 **GP-간호사-환자 구두 협의 + 케어플랜 반영**이 실질적 안전 경로
- 재택 방문 시 약물 변경사항을 케어플랜에 즉시 기록·공유하는 체계 확보

**한국 적용 주의:**
- 노르웨이 지자체 기반 가정간호 시스템 — 한국 방문간호·재택의료 구조와 상이 [조건부 외삽]
- 국내 방문간호 기록 체계·GP 연계 강화가 유사 간극 해소 방향

---

## 장기 PPI 감약 — 환자+GP 병행 개입이 가장 효과적 (JAMA Intern Med 2026) [CLINICAL]

> [출처: Fournier JP et al. Deprescribing Intervention and Reduction of Proton Pump Inhibitor Use in Primary Care: A Cluster Randomized Clinical Trial. JAMA Intern Med. 2026;186(6):668-676. PMID:41973459, DOI:10.1001/jamainternmed.2026.0584]
> 클러스터 RCT (1:1:1), n=1,498 GP·34,409 환자, 프랑스 서부 2개 지역, 2020-2021.

**대상**: 성인 ≥1년 PPI 사용자 (부적절 장기 사용)

**3군 설계:**
1. **환자+GP 병행 개입**: 환자 직접 우편 교육 브로셔 + GP에 감약 알고리즘 편지
2. **GP 단독 개입**: GP에만 알고리즘 편지
3. **일반치료 (대조군)**

**핵심 결과 (PPI 50% 이상 감량 달성률):**

| 군 | 감량 달성률 | 조정 절대차 |
|---|---|---|
| 환자+GP 병행 | **14.9%** | +6.9% (vs 대조군) |
| GP 단독 | 7.7% | — |
| 일반치료 | 7.0% | (기준) |

- **GERD 증상(GIS 점수)**: 세 군 간 유의 차이 없음 → 증상 악화 우려 근거 없음

**임상 적용:**

1. 외래 장기 PPI 환자 감약 상담 시 환자에게 **직접 교육 자료 제공** 이 핵심 (GP 설명 단독보다 효과 우월)
2. "PPI 끊으면 속 쓰릴까봐 걱정" 호소 환자 → "GERD 증상 악화 없음" 대규모 RCT 근거 직접 제시
3. 감약 알고리즘: 적응증 없는 장기 PPI → 격주 감량 → 중단 시도 → 재발 시 PRN 전환

**감약 적응증 확인 (장기 PPI 재검토 기준):**
- 적응증 있는 PPI 유지: 활동성 GERD·위궤양·NSAID 병용·H.pylori 박멸 후 고위험군
- **재검토 대상**: 적응증 없는 예방 목적 장기 사용 (≥1년)

관련: [[functional-dyspepsia]] — 기능성 소화불량·GERD 감별
관련: [[prescribing-cascade]] — PPI 추가 cascade (NSAID → 위장 → PPI → 마그네슘 결핍 → 보충제)

---

## 고독감 스크리닝 → 부적절 약물 우선 감약 대상 선별 — 고령 여성 특이적 연관 (관찰 연구) [CLINICAL — 조건부]

> [출처: Savage RD et al. Loneliness and Initiation of Potentially Inappropriate Pain and Psychotropic Medications: A Retrospective Cohort Study. Drugs Aging. 2026;DOI:10.1007/s40266-026-01324-7. PMID:42536335]
> 후향적 코호트 연구. 캐나다 지역사회 거주 ≥66세 온타리오 주민 n=2,348 (여성 54.6%, 평균 75.4세). 2008–2009년 Canadian Community Health Survey-Healthy Aging 응답자 → 건강기록 연계, 3년 추적.
> [초록 기반 — 전문 미확인]

**핵심 발견:**

고독감(Three-Item Loneliness Scale ≥6)이 있는 고령 **여성**은, 고독감 없는 여성 대비 **2019 AGS Beers 기준 부적절 진통제** 신규 시작률이 시간 경과에 따라 급격히 증가:

| 추적 시점 | HR (95% CI) |
|---|---|
| 90일 | 1.57 (0.96–2.27) |
| 630일 | 1.52 (1.05–2.12) |
| **1080일** | **3.22 (1.22–7.78)** |

- Beers 기준 부적절 정신과약(psychotropic) 신규 시작도 증가 경향 (추정치 정밀도 낮음)
- **고독감 있는 남성**: 유의한 연관 없음 — **성별 이질성(sex interaction)** 확인

**전체 코호트에서 고독감 유병률**: 12.3% (383명/2,348명)

**Beers PIM 정의**: 2019 AGS Beers Criteria — 부적절 진통제 + 부적절 정신과약

**임상 함의 — 감약 우선 대상 선별:**

1. **외래 고령 여성 환자** → 3문항 외로움 척도 스크리닝 → 고독감 양성이면 현재 복용 진통제·정신과약 Beers 기준 적합성 재검토 우선화
2. "통증으로 약을 시작했다" → 고독감이 통증의 사회적 증폭(social amplification) 요인으로 작용하고 있을 가능성 고려
3. 외로운 여성 환자에서 Beers PIM 처방이 시간에 따라 누적되는 경향 → 장기 추적 환자 약물 재검토 시 고독 스크리닝 추가
4. 남성에서 연관 없음: 성별 맥락 고려 — 동일 고독감도 여성에서 처방 행동 변화로 귀결되는 기전 연구 필요

**외래 적용 흐름 제안 (논문 저자 권고 기반):**
- 외로움 스크리닝 (3문항) → Beers/STOPP 기반 약물 재검토 → 감약/대체 검토 → 고독감 근본 원인 개입 (사회처방·지역사회 프로그램 연계)

**주의사항:**
- 후향적 코호트 연구 → 인과관계 방향성 확정 불가 (역인과: 통증·정신과 문제 때문에 고독해졌을 가능성도 존재)
- 캐나다 온타리오 코호트 — 한국 지역사회 노인 외삽 시 사회 문화적 차이 고려 [조건부 외삽]
- 정신과약 추정치 정밀도 낮음 (표본 수 제한)

관련: [[fall-prevention-awv]] — FRIDs 낙상 위험 약물 (Beers 기준과 중복 대상 多)
관련: [[elderly-psychotropic-deprescribing]] — 노인 정신과약 감약 프로토콜
관련: [[depression-screening]] — 고령 여성 우울·사회적 결정인자 스크리닝

---

## STOPP / START / Beers 핵심 도구 소개 [CLINICAL]

> [3대 도구 개요 — 가이드라인 default 인용: McDonald EG et al. Can Fam Physician 2026 PMID:41844291 + AGS Beers Criteria 2023 + O'Mahony D et al. STOPP/START v3 2023]

### STOPP v3 (Screening Tool of Older Person's Prescriptions)

- **목적**: 노인에서 **피해야 할** 잠재적 부적절 처방(PIP) 탐지
- **대상**: ≥65세 다약제 환자
- **방식**: 각 약물·질환 조합에 대한 "이 약은 이 환자에게 적절한가?" 체크리스트
- **v3 (2023)**: 약 190개 기준 — BZD·항콜린·NSAID·오피오이드·혈당강하제 등

### START v3 (Screening Tool to Alert to Right Treatment)

- **목적**: 노인에서 **빠져있는** 유익한 처방 감지 (처방 누락, undertreatment)
- **방식**: "이 환자에게 빠진 약이 없는가?" 체크리스트
- **예시 항목**: ACEi/ARB (심부전·신장보호), 스타틴 (기대 여명 적절 시), 칼슘+D (골다공증), 흡입 기관지확장제 (COPD)

### Beers Criteria 2023 (AGS)

- **목적**: 노인에서 일반적으로 피해야 할 약물 목록 (미국 AGS 기준)
- **미국 기준** — 한국 가용 약물·보험 체계와 부분 불일치 [한국 적용 시 출처 미확인 항목 존재]
- **대표 포함 약물**: 1세대 항히스타민·항콜린·BZD·Z-drug·아미트립틸린(저용량 포함)·diphenhydramine·meperidine·carbamezepine·dipyridamole 등
- **Beers vs STOPP 차이**: Beers는 약물 목록 중심; STOPP은 약물+임상 맥락 기반

### 3대 도구 비교 요약

| 도구 | 기원 | 방향 | 강점 |
|---|---|---|---|
| STOPP | 유럽(아일랜드) | 과처방(overtreatment) 탐지 | 임상 맥락(진단+약물) 결합 |
| START | 유럽(아일랜드) | 과소처방(undertreatment) 탐지 | 누락 처방 발견 |
| Beers | 미국(AGS) | 과처방 탐지 | 미국 표준, 널리 알려짐 |

### 약사-의사 처방 최적화 (Medication Reconciliation)의 의미

> 미르 반응(2026-08-30): "약사-의사 처방최적화는 무슨의미지?"

- **처방 최적화(prescription optimization/medication reconciliation)** = 환자가 복용하는 모든 약물을 구조적으로 검토해 불필요한 약 중단, 용량 조정, 상호작용 해소, 누락 처방 추가를 수행하는 과정
- 약사 참여가 핵심: 의사는 임상 판단 + 약사는 약리 전문성 → 협력 시 단독 의사 검토보다 PIP 발견율 ↑
- **환자·의사의 불안 해소 접근**: "약을 줄이면 병이 나빠질까?"는 흔한 걱정. 해결 전략:
  1. **근거 먼저**: "이 약은 현재 적응증이 없습니다 / 이 용량은 낙상 위험이 있습니다" — 이유 설명
  2. **점진적 감량**: 급격한 중단이 아닌 gradual taper으로 불안 완화
  3. **다시 처방 가능**: "약을 줄였다가 증상이 다시 생기면 언제든 다시 처방합니다"
  4. **목표 공유**: "약을 줄이는 이유는 부작용(낙상·인지저하·신장기능)을 예방하기 위해서"

---

## 한국 NHIS 다약제 관리 프로그램 — 재입원 15% 감소, 비용편익비 3.8 (Drugs Aging 2026) [CLINICAL]

> [출처: (저자 미확인 — 초록 기반) Drugs & Aging 2026 Aug 8. PMID:42570168]
> 한국 건강보험심사평가원 시범 다약제 관리 프로그램 (2020-21, 34개 병원), ≥65세·≥10종 약물 입원 환자

### 프로그램 개요

- 주관: 건강보험심사평가원(HIRA)
- 기간: 2020-2021 (시범)
- 규모: 34개 병원
- 대상: ≥65세 입원 환자 + ≥10종 약물 (또는 ≥5종 + 고위험 인자)
- 중재: **약사-의사 협력 처방 최적화** (입원 중 리뷰 + 퇴원 계획 연계)

### 결과

| 지표 | 결과 |
|---|---|
| **90일 재입원** | **aHR 0.85 (15% 감소)** |
| 응급실 방문 | 유의한 차이 없음 |
| 입원 의료비 | USD $340 절감/환자 |
| **비용편익비** | **3.8 (1원 투자 → 3.8원 절감)** |

### 외래 적용 포인트

- **적용 기준**: ≥65세 + ≥10종 약물 (또는 5종 + 고위험) → 처방 재검토 강력 권고
- "다약제 관리가 실제로 재입원을 줄인다" — 한국 데이터 근거
- 비용편익 3.8: 보험자(건강보험공단) 관점에서 경제적 정당성 확보; 외래에서 "왜 약을 줄여야 하는가" 설명 근거

### STOPP/START 연계

- 한국 NHIS 프로그램은 STOPP/START 및 Beers 기준 기반 구조적 접근 연계
- 이 프로그램 데이터 = 위 STOPP/START/Beers 도구의 한국 실효성 간접 검증

### 한계

- 34개 병원 시범 → 전체 한국 의료 대표성 제한
- 입원 환자 대상 → 외래 다약제 환자 적용 시 외삽 주의
- 응급실 감소는 없음 (입원만 감소) — 편익 범위 제한적

---

## 노인 오피오이드 안전 처방 [CLINICAL] (2026-09-15 추가)

> 출처 (Tier 2): Ribeiro H et al. Drugs Aging. 2026. DOI:10.1007/s40266-026-01329-2. PMID:42627454

**배경:** 노인에서 오피오이드는 약동학 변화로 위험↑ — 신·간 청소율↓, 체지방↑, 단백결합↓

### 노화에 따른 약동학 변화 (요약)

| 변화 | 임상 의미 |
|---|---|
| 신장·간 청소율 ↓ | 반감기 연장 → 용량 조절 + 투여 간격 연장 |
| 체지방 ↑ | 지용성 약물 분포용적↑ (지속성↑) |
| 알부민 ↓ | 유리형 오피오이드↑ → 효과/부작용 증폭 |
| 근육량 ↓ | 용량 기준(kg) 재검토 |

### 처방 원칙 ("start low, go slow")

- **비활성 대사체 선호**: morphine의 M6G 활성 대사체 축적 위험 → hydromorphone·fentanyl 등 비활성 대사체 선호
- 초기 용량: 성인 표준 용량의 **25-50%부터 시작**
- **증량 간격 연장**: 신기능에 따라 투여 간격 조정
- 정기 재평가: 통증 조절·기능·부작용 삼각 모니터링

### 선제적 부작용 관리 (노인 필수)

1. **변비** — 오피오이드 시작 시 완하제 동시 처방 (대기 처방 아님)
2. **낙상 위험** — 오피오이드+항우울제 중복 시 낙상 위험 aOR 1.40-1.45 (PMID:42343007 참조)
3. **인지 저하** — 시작 2-4주 내 인지 변화 모니터링 (Mini-Cog 재평가 고려)

### 암성 vs 비암성 통증 구분

| 구분 | 접근 |
|---|---|
| **암성 통증** | 강효 오피오이드 우선 + 경피(transdermal) 고정용량 + ultra-rapid acting fentanyl 돌발통증 |
| **비암성 만성통증** | 병리 기전 특이 치료 우선 → 오피오이드는 **시간 제한** 사용, 위험-편익 평가 후 |

> ⚠ 비암성 만성통증에서 "영구 처방" 금지 — 치료 목표(기능 회복) + 시간 제한 명시 후 시작

### 관련

- [[fall-prevention-awv]] — FRID 낙상 입원 위험 (PMID:42343007)
- [[elderly-psychotropic-deprescribing]] — 심리작용약 감약

