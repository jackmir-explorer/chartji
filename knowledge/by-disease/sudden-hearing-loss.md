# 돌발성 난청 (Sudden Sensorineural Hearing Loss, SSNHL)

keywords: 돌발성난청, sudden hearing loss, SSNHL, sudden sensorineural hearing loss, 메칠론, MPD, ITS, intratympanic steroid

---

## 정의 (definition)

72시간 이내 발생한 ≥30 dB 감각신경성 난청 (3개 이상 인접 주파수). 응급 — 발생 후 가능한 빨리(이상적으로 14일 이내) 치료 시작 시 회복률 ↑.

---

## 문진/검사 (exam)

### 증상

- **갑작스러운 편측 청력 저하** (보통 아침에 일어나니 안 들림)
- 이명·이충만감 동반 흔함
- 어지럼증 동반 시 예후 불량

### 진단

- 청력검사 (PTA) — 즉시 시행
- 고막 시진 정상 (전음성 난청 배제)
- MRI internal auditory canal — vestibular schwannoma 배제 (특히 이명 동반)

### 동반 평가 [TIPS — by ENT 교수]

- **당뇨 확인 — 스테로이드 처방 전 필수**
- 고혈압·심혈관 위험 평가

---

## 처방/치료 (protocol)

### 표준 — 경구 스테로이드 [TIPS — by ENT 교수]

- **Methylprednisolone (메칠론) 8T#2 → 7일 tapering**
  - 처방 표기: 시작 용량 → 1~2일 단위로 감량
  - 가능한 빨리 시작 (발생 후 14일 이내가 효과 큼)

### Intratympanic Steroid (ITS) [TIPS — by ENT 교수]

- 호전을 빠르게 하고 싶을 때 추가
- 적응: 경구 스테로이드 단독 미반응 / 빠른 회복 필요 / DM·궤양 등 경구 부담
- ENT 시술

### 보조

- 산소·hyperbaric oxygen (논쟁 중, 일부 가이드라인 권장)
- 생활습관 — 안정·스트레스 관리

---

## 주의 (precaution)

### 스테로이드 처방 전 필수 확인 [TIPS — by ENT 교수]

- **당뇨**: 혈당 상승 — DM 환자도 처방 가능하지만 혈당 모니터링·환자 교육 필수
- **녹내장·황반변성**: 스테로이드 외 옵션 고려 (특히 황반변성)
- 위궤양·소화기 출혈 과거력
- 정신질환·BPSD
- 활동성 감염

---

## 비고 (notes)

- 자연 회복률 ~30~65% (변동성 큼)
- 어지럼증 동반·심한 청력 손실·고령 시 예후 불량
- 30일 후 청력 plateau — 회복 평가 시점
- 보청기 적응 시점 — 6개월 후 청력 안정 시 고려

---

## 의뢰 기준 (referral)

- **돌발성 난청 의심 즉시 ENT 의뢰** — 시간이 회복률 결정
- ITS 시술 필요 → ENT
- 어지럼증 심한 동반 → 신경이과·내이 진단
- vestibular schwannoma 의심 (편측·이명 동반) → MRI IAC + ENT
