...
- **예외 — 자동 routine 산출물 (2026-04-22 확정)**: `routines/deep-extract.md`·`routines/scout.md` 등 **자동 routine이 생성하는 md 파일 반영**은 PR 없이 **main 직접 반영**. 미르가 쓴 💬 반응 한 문장(2026-07-16 답변 게이트) 자체가 ingest 승인 신호이므로 추가 게이트 불필요. 과거 routine 명세의 "PR 생성 후 미르 승인" 경로는 폐기. (**scout 도 2026-07-24 부터 PR 폐지** — 완료 알림이 PR 알림을 대체하므로 GitHub MCP 로 main 직접 write. 샌드박스 git-push 차단 우회는 CLAUDE.md 「403 우회」 참조.)
