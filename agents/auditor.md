...
| **entry 주제 부조화** (2026-05-06 신설 — heart-failure-volume-overload 사건 후속) | 한 bundle entry가 의미상 별개의 주제를 함께 보유한 경우 → 검색·LLM inject 시 잘못된 매칭. 검출 신호: ① entry 주석에 "+", "통합", "별도 topic 키로 격리", "보완 누적" 등 자백 문구 ② primarySources가 서로 다른 주제·시점의 논문들로 구성 + sections이 그 주제를 각각 별도 다룸 ③ keywords에 이질적 도메인(진단·질환·치료) 혼재. 자동 수정 금지 — 분리 제안만, 분할 실행 권한은 미르 승인 후 Auditor (앞 항목 R2 적용). 분리 후 bundle 재컴파일은 Liby. |
| **키 중복 할당 (hard-check)** (2026-05-07 신설 — 20건 데이터 손실 사건 후속) | `KNOWLEDGE_BUNDLE["X"] = ...` 패턴이 동일 키에 2회 이상 등장 + 두 할당의 우변이 다른 객체일 때 후자가 전자를 덮어써 데이터 손실. 휴리스틱 아닌 **hard-check** (grep + 구문 분석으로 100% 검출). 즉시 조치 권고: 통합 / 분리 / 폐기 중 결단. 자동 수정 금지 — 컨텐츠 결단은 미르. |
